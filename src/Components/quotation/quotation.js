...
function Quotation(props) {
  ...
  useEffect(() => {
    const fetchlatest_invoice = async () => {
      const response = await fetch(
        `${route}/api/purchase-quotation/${selected_branch.id}/latest-quotation/`,
        {
          headers: { Authorization: `Bearer ${user.access}` },
        }
      );
      ...
      if (response.ok) {
        setinvoice(json.quotation_number);
      }
      if (!response.ok) {
        went_wrong_toast();
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected_branch) {
      ...
      const response = await fetch(`${route}/api/purchase-quotation/`, {
        method: "POST",
        headers: {
