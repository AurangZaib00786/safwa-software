...
function Dailymeal_edit(props) {
  ...
  const [table_data_copy, settable_data_copy] = useState(
    saved_data?.building_details
  );
  ...
  useEffect(() => {
    ...
    settable_data({
      ...
      data: saved_data?.building_details,
    });

    const new_columns =
      saved_data?.building_details?.length > 0
        ? saved_data?.building_details[0]
        : null;
  ...
  const fetchmeal = async (id) => {
    ...
    if (response.ok) {
      ...
      settable_data({
        type: "Set_product_history",
        data: json?.building_details,
      });
      settable_data_copy(json?.building_details);
      const new_columns =
        json?.building_details?.length > 0 ? json?.building_details[0] : null;
      setcolumn(new_columns?.pot_details);
      setcolumn_copy(new_columns?.pot_details);
