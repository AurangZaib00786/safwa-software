...
import Typeform from "./expensetypeform";
import UpdateType from "./updateexpensetypeform";
...
export default function Expense_Type(props) {
  ...
  const all_types = props.state.Settablehistory.table_history;
    ...
    return (
      <span className="action d-flex">
        <IconButton
          ...
          onClick={() => {
            setrow_id(row.id);
            seturl_to_delete(`${route}/api/expense-types/${row.id}/`);
            setdelete_user(true);
          }}
        >
  ...
  const makepdf = () => {
    const body = all_types.map((item, index) => {
    ...
    const documentDefinition = {
      content: [
        { text: "Expenses Type", style: "header" },
  ...
  const download = () => {
    ...
    pdfMake.createPdf(documentDefinition).download("Expense type.pdf");
  };
