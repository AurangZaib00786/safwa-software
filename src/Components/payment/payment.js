...
export default function Payment(props) {
  ...
  useEffect(() => {
    dispatch({ type: "Set_menuitem", data: "payment_customer" });
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchpaments = async () => {
      var url = `${route}/api/parties-payments/?account_head=${selected_branch.id}&user_id=${current_user.id}&user_type=${current_user?.profile?.user_type}&party_type=Customer&reference=CB&start_date=${start_date}&end_date=${end_date}`;
  ...
  const makepdf = () => {
    ...
    const documentDefinition = {
      content: [
        { text: "Customer Payments", style: "header" },
  ...
  const download = () => {
    ...
    pdfMake.createPdf(documentDefinition).download("Customer payments.pdf");
  };
