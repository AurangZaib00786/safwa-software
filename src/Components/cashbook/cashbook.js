...
export default function Cashboook(props) {
  ...
  const [all_banks, setall_banks] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchpaments = async () => {
      var url = `${route}/api/cashbook/?account_head=${selected_branch.id}&user_id=${current_user.id}&user_type=${current_user?.profile?.user_type}&start_date=${start_date}&end_date=${end_date}`;
    ...
    const fetchWorkouts = async () => {
      var url = `${route}/api/banks/?account_head=${selected_branch.id}`;
      if (!settings?.user_base?.account_base) {
        if (current_user.profile.user_type === "user") {
          url = `${route}/api/banks/?user_id=${current_user.profile.parent_user}`;
        } else {
          url = `${route}/api/banks/?user_id=${current_user.id}`;
        }
      }
    ...
    return (
      <span className="action d-flex">
        <IconButton
          ...
          onClick={() => {
            setrow_id(row.id);
            seturl_to_delete(`${route}/api/cashbook/${row.id}/`);
            setdelete_user(true);
          }}
        >
        ...
        <IconButton
          ...
          onClick={() => {
            ...
            if (row.type === "cash_in") {
              settype({ value: row.type, label: "Cash In" });
            } else if (row.type === "cash_out") {
              settype({ value: row.type, label: "Cash Out" });
            }
            setbank({ value: row.bank, label: row.bank_name });
            setid(row.id);
  ...
  const type_formatter = (cell, row, rowIndex, formatExtraData) => {
    if (cell === "cash_in") {
      return "Cash In";
    } else if (cell === "cash_out") {
      return "Cash Out";
    }
  };
  const columns = [
  ...
  const makepdf = () => {
    ...
    const documentDefinition = {
      content: [
        { text: "type saleman", style: "header" },
  ...
  const download = () => {
    ...
    pdfMake.createPdf(documentDefinition).download("type payments.pdf");
  };
  ...
  const handleSubmit_update = async (e) => {
    ...
    const response = await fetch(`${route}/api/cashbook/${id}/`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${user.access}`,
      },
      ...
      <div className="card">
        <form onSubmit={handlesubmit}>
          <div className="card-header d-flex justify-content-between bg-white">
            <h3 className="mt-2 me-2">Add Cashbook</h3>
