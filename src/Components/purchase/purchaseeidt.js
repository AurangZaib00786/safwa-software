...
function Purchase_Edit(props) {
  ...
  const [refernce_invoice, setrefernce_invoice] = useState(
    id_data?.reference_invoice
  );
  const [supplier, setsupplier] = useState({
    value: id_data?.supplier,
    label: id_data?.supplier_name,
  });
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      data.forEach((item) => {
        ...
        discount_total += Number(item.discount_amount);
        total_total += Number(item.total);
      });
      setsub_total_total(subtotal_total);
      settotal_discount(discount_total);
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected_branch && table_data) {
      ...
      const response = await fetch(`${route}/api/purchases/${id_data?.id}/`, {
        method: "PATCH",
        headers: {
        ...
        body: JSON.stringify({
          ...
          tax_percentage: tax_percentage ? tax_percentage : 0,
