...
function Purchase_history(props) {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      var url = `${route}/api/purchases/?account_head=${selected_branch.id}&start_date=${start_date}&end_date=${end_date}`;
      if (
        date_range[0].endDate.getFullYear() -
          date_range[0].startDate.getFullYear() ===
        10
      ) {
        url = `${route}/api/purchases/?account_head=${selected_branch.id}`;
      }
  ...
  const linkFollow = (cell, row, rowIndex, formatExtraData) => {
    return (
      <span className="action d-flex">
        ...
        {!current_user?.permissions?.includes("can_delete") && (
          <IconButton
        ...
        {!current_user?.permissions?.includes("can_edit") && (
          <IconButton
            className="p-0"
