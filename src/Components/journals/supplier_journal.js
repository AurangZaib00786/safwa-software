...
export default function Supplier_Journal(props) {
  ...
  useEffect(() => {
    ...
    const fetchpaments = async () => {
      const response = await fetch(
        `${route}/api/parties-payments/?account_head=${selected_branch.id}&user_id=${current_user.id}&user_type=${current_user?.profile?.user_type}&party_type=Supplier&reference=JV`,
        {
          headers: { Authorization: `Bearer ${user.access}` },
        }
      );
  ...
  const makepdf = () => {
    ...
    const documentDefinition = {
      content: [
        { text: "Vendors Journal", style: "header" },
  ...
  const download = () => {
    ...
    pdfMake.createPdf(documentDefinition).download("Vendors Journal.pdf");
  };
