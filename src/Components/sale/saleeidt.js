...
function Sale_Edit(props) {
  ...
  useEffect(() => {
    ...
    setpayment_type({
      value: current_data.payment_type,
      label: current_data.payment_type,
    });
    setsaleman({
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected_branch && table_data) {
      ...
      const response = await fetch(`${route}/api/sales/${id}/`, {
        method: "PATCH",
        headers: {
    ...
    if (column.dataField === "quantity") {
      ...
      if (
        ...
        if (calculate_total_pr_item > item.offer_rate) {
          ...
          Red_toast(
            `Product final amount ${calculate_total_pr_item} is less than offer rate ${item.offer_rate}`
          );
      ...
      if (calculate_total_pr_item > item.offer_rate) {
        ...
        item["discount_percentage"] = oldValue;
        settable_data({ type: "Update_product_history", data: item });
      }
          ...
          <div className="card">
            ...
            <div className="card-body  ">
              <div className="row  d-sm-flex  mt-1">
                ...
                <div className="ol-6 col-md-2 mb-2">
                  <Select
                    className={
                      saleman !== ""
