...
import Payment_typeform from './payment_typeform'
import Payment_typeUpdate from './updatepayment_typeform';
...
export default function Payment_type(props) {
  const {t} = useTranslation()
    ...
    useEffect(() => {
      setisloading(true)
      dispatch({type:"Set_menuitem", data:'payment_type'})
      const fetchWorkouts = async () => {
        const response = await fetch(`${route}/api/payment-types/?branch_id=${selected_branch.id}`, {
    ...
    const Action = (cell, row, rowIndex, formatExtraData) => {
      return (
          <span className='action d-flex'  >
              <IconButton className='border border-danger rounded me-2' onClick={()=>{
                  setrow_id(row.id)
                  seturl_to_delete(`${route}/api/payment-types/${row.id}/`)
                  setdelete_user(true)
                  }}>
              ...
              <EditOutlinedIcon className='m-1'  style={{color:'#003049'}} fontSize="small"/>
              </IconButton>
      </span>
      );
    };
    ...
    const columns = [
      ...
      { dataField: 'name', text: t('name'),sort: true,headerFormatter:headerstyle},
    ...
    const options = {
        ...
        sizePerPageList: [{
            ...
            text: 'All', value: all_customers.length
    ...
    const makepdf=()=>{
      ...
      const documentDefinition = {
        content: [
          { text: 'Payment Types', style: 'header' },
    ...
    const download=()=>{
      ...
      pdfMake.createPdf(documentDefinition).download('paymenttypes.pdf');
    }
  ...
  return (
    <div className='p-3'>
        <h1 className='mb-3' style={{fontSize:'1.8rem', fontWeight:'normal'}}>{t('side_bar_paymnettype')}</h1>
        <div className='card'>
            <div className='card-header bg-white  d-flex justify-content-between '>
            <h1 className='mb-3' style={{fontSize:'1.3rem', fontWeight:'normal'}}>{t('paymenttype_list')}</h1>
                <Button type='button' className='mb-2' variant='outline-success'  onClick={()=>setshowmodel(!showmodel)}><FontAwesomeIcon icon={faUserPlus} />   {t('add_paymenttype')}</Button>
            </div>
            ...
            <ToolkitProvider
                ...
                >
                {
                    ...
                    <div>
                        ...
                        <div>
                              <ExportCSVButton { ...props.csvProps } className='csvbutton  border bg-secondary text-light me-2  mb-2'>Export CSV</ExportCSVButton>
      ...
      />}
        {showmodelupdate && <Payment_typeUpdate
