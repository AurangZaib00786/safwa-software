...
function Sale_Quotation(props) {
  ...
  useEffect(() => {
    const fetchcompany = async () => {
      const response = await fetch(
        `${route}/api/companies/?account_head=${selected_branch.id}`,
        {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      if (company.value !== "all") {
        url = `${route}/api/stock/?account_head=${selected_branch.id}&company_id=${company.value}`;
      }
  ...
  }, [selected_branch, check_status, company]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected_branch && table_data) {
      ...
      const response = await fetch(`${route}/api/sale-quotation/`, {
        method: "POST",
        headers: {
