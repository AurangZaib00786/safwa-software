...
function Sale_Return_History(props) {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      var url = `${route}/api/sale-return/?account_head=${selected_branch.id}&user_id=${current_user.id}&user_type=${current_user?.profile?.user_type}&start_date=${start_date}&end_date=${end_date}`;
      if (
        date_range[0].endDate.getFullYear() -
          date_range[0].startDate.getFullYear() ===
        10
      ) {
        url = `${route}/api/sale-return/?account_head=${selected_branch.id}&user_id=${current_user.id}&user_type=${current_user?.profile?.user_type}`;
      }
    ...
    return (
      <span className="action d-flex">
        ...
        <IconButton
          ...
          onClick={() => {
            setrow_id(row.id);
            seturl_to_delete(`${route}/api/sale-return/${row.id}/`);
            setdelete_user(true);
          }}
        >
  ...
  }, [sale_person, customer, date_range, status]);
  ...
  const makepdf = () => {
    ...
    const documentDefinition = {
      content: [
        { text: "Sale Return History", style: "header" },
        {
          layout: "noBorders",
          table: {
            headerRows: 1,
  ...
  const download = () => {
    ...
    pdfMake.createPdf(documentDefinition).download("Salereturnhistory.pdf");
  };
...
export default memo(Sale_Return_History);
